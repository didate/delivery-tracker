...
export class DeliveryItemService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/delivery-items');
  ...
  update(deliveryItem: IDeliveryItem): Observable<EntityResponseType> {
    return this.http.put<IDeliveryItem>(
      `${this.resourceUrl}/${encodeURIComponent(this.getDeliveryItemIdentifier(deliveryItem))}`,
      deliveryItem,
      { observe: 'response' },
    );
  }
  ...
  partialUpdate(deliveryItem: PartialUpdateDeliveryItem): Observable<EntityResponseType> {
    return this.http.patch<IDeliveryItem>(
      `${this.resourceUrl}/${encodeURIComponent(this.getDeliveryItemIdentifier(deliveryItem))}`,
      deliveryItem,
      { observe: 'response' },
    );
  ...
  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IDeliveryItem>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' });
  }
