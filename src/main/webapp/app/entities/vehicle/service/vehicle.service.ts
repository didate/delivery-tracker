...
export class VehicleService {
  ...
  update(vehicle: IVehicle): Observable<EntityResponseType> {
    return this.http.put<IVehicle>(`${this.resourceUrl}/${encodeURIComponent(this.getVehicleIdentifier(vehicle))}`, vehicle, {
      observe: 'response',
    });
  }

  partialUpdate(vehicle: PartialUpdateVehicle): Observable<EntityResponseType> {
    return this.http.patch<IVehicle>(`${this.resourceUrl}/${encodeURIComponent(this.getVehicleIdentifier(vehicle))}`, vehicle, {
      observe: 'response',
    });
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http.get<IVehicle>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' });
  }
