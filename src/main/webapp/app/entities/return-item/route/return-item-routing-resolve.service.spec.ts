...
import returnItemResolve from './return-item-routing-resolve.service';

describe('ReturnItem routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IReturnItem returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          returnItemResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IReturnItem>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
