...
import tenantSettingsResolve from './tenant-settings-routing-resolve.service';

describe('TenantSettings routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ITenantSettings returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          tenantSettingsResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<ITenantSettings>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
