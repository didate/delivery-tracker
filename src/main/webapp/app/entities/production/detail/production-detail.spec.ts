...
describe('Production Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./production-detail').then(m => m.ProductionDetail),
              resolve: { production: () => of({ id: 25690 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load production on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', ProductionDetail);

      // THEN
      expect(instance.production()).toEqual(expect.objectContaining({ id: 25690 }));
    });
  });
