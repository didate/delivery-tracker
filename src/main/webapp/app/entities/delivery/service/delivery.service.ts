...
export class DeliveryService {
  ...
  update(delivery: IDelivery): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(delivery);
    return this.http
      .put<RestDelivery>(`${this.resourceUrl}/${encodeURIComponent(this.getDeliveryIdentifier(delivery))}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  partialUpdate(delivery: PartialUpdateDelivery): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(delivery);
    return this.http
      .patch<RestDelivery>(`${this.resourceUrl}/${encodeURIComponent(this.getDeliveryIdentifier(delivery))}`, copy, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }

  find(id: number): Observable<EntityResponseType> {
    return this.http
      .get<RestDelivery>(`${this.resourceUrl}/${encodeURIComponent(id)}`, { observe: 'response' })
      .pipe(map(res => this.convertResponseFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IDelivery | NewDelivery | PartialUpdateDelivery>(delivery: T): RestOf<T> {
    return {
      ...delivery,
      deliveryDate: delivery.deliveryDate?.format(DATE_FORMAT) ?? null,
    };
  }
