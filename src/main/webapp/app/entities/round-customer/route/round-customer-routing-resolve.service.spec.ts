...
import roundCustomerResolve from './round-customer-routing-resolve.service';

describe('RoundCustomer routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IRoundCustomer returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          roundCustomerResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      // GIVEN
      vitest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IRoundCustomer>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
