...
describe('Round Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./round-detail').then(m => m.RoundDetail),
              resolve: { round: () => of({ id: 19799 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load round on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', RoundDetail);

      // THEN
      expect(instance.round()).toEqual(expect.objectContaining({ id: 19799 }));
    });
  });
